---
title: "The PEER Model"
image: "../images/peer.png"
image-width: "20rem"
#date: last-modified

page-layout: article
toc: false
suppress: true 
---

{{< include /_prefix.qmd >}}

<!-- Welcome to PEER! -->
::: {.quarto-listing .quarto-listing-container-grid }
::: {.list .grid .quarto-listing-cols-2 .bg-info-light style="border-radius:10px;"}
::: {.g-col-1 style="padding-left:10px;"}

# The PEER Model

:::

::: {.g-col-1 style="padding-right:10px;"}

## What is PEER?

At PEER Field Schools, you'll delve into research project development and best practices, engage with like-minded professionals, and gain invaluable insights into the world of educational research. Our expert facilitators will guide you through hands-on activities and collaborative discussions, helping you shape your project and engage your passion. 

PEER will boost your education research skills and push your project forward!

:::

:::

:::
<!-- END -->


:::{.quarto-listing .quarto-listing-container-grid .list .grid .quarto-listing-cols-2 .bg-info style="border-radius:10px;padding-left:10px;"}

:::{.g-col-1}

# PEER Principles

At PEER, you will learn fundamental and applied research skills in a supportive and participatory research environment. We believe research is: 

:::

:::{.g-col-1}

:::{.quarto-listing .quarto-listing-container-grid .list .grid .quarto-listing-cols-2}

:::{.g-col-1 .card .card-body .text-center .bg-info .border-0}

:::{.bi .bi-arrow-repeat style="font-size: 70px"}

:::
::: {style="font-size: 150%;"}
**Responsive**
:::
Field schools respond to participant needs
:::

:::{.g-col-1 .card .card-body .text-center .bg-info .border-0}

:::{.bi .bi-balloon-fill  style="font-size: 70px"}

:::

::: {style="font-size: 150%;"}
**Playful**
:::
Researchers play with ideas and data to generate new knowledge
:::

:::{.g-col-1 .card .card-body .text-center .bg-info .border-0}

:::{.bi .bi-envelope-paper-fill style="font-size: 70px"}

:::

::: {style="font-size: 150%;"}
**Communicative**
:::
Writing and presenting are core parts of research
::: 

:::{.g-col-1 .card .card-body .text-center .bg-info .border-0}

:::{.bi .bi-person-plus-fill style="font-size: 70px"}

:::

::: {style="font-size: 150%;"}
**Collaborative**
:::
People do research together, in communities

:::
:::
:::
:::

:::{.bg-info style="border-radius:10px;padding:10px;padding-top:20px;margin-top:20px;"}

## We believe research is:


:::{.quarto-listing .quarto-listing-container-grid .list .grid .quarto-listing-cols-4}

:::{.g-col-1 .card .card-body .text-center .bg-info .border-0}

:::{.bi .bi-arrow-repeat style="font-size: 70px"}

:::
::: {style="font-size: 150%;"}
**Responsive**
:::
Field schools respond to participant needs
:::

:::{.g-col-1 .card .card-body .text-center .bg-info .border-0}

:::{.bi .bi-balloon-fill  style="font-size: 70px"}

:::

::: {style="font-size: 150%;"}
**Playful**
:::
Researchers play with ideas and data to generate new knowledge
:::

:::{.g-col-1 .card .card-body .text-center .bg-info .border-0}

:::{.bi .bi-envelope-paper-fill style="font-size: 70px"}

:::

::: {style="font-size: 150%;"}
**Communicative**
:::
Writing and presenting are core parts of research
::: 

:::{.g-col-1 .card .card-body .text-center .bg-info .border-0}

:::{.bi .bi-person-plus-fill style="font-size: 70px"}

:::

::: {style="font-size: 150%;"}
**Collaborative**
:::
People do research together, in communities

:::
:::
:::

::: {.quarto-listing .quarto-listing-container-grid }
::: {.list .grid .quarto-listing-cols-2 style="border-radius:10px; margin-top: 20px;height:500px;"}
::: {.g-col-1 style="padding-left:10px;"}



## Who Benefits From the PEER program?

PEER participants come from a wide range of science backgrounds and professions. Participants include:  

* Discipline-based education researchers looking to expand their methodological repertoire
* Disciplinary faculty just getting started on the scholarship of teaching and learning
* Education faculty looking to join a research group and access video-based data
* Faculty development experts learning to mentor faculty through scholarship of teaching and learning (SoTL) projects 
* Graduate students and postdoctorates who need mentoring and community support for their research projects
* Undergraduate students who attend field schools with their advisors

:::

::: {.g-col-1 .text-center style="margin-left:40px;margin-right:40px;background-image:url(../images/action-shots/facilitator_explaining.png);background-repeat:no-repeat;background-size: cover;position: relative;"}

:::{style="position:absolute;bottom:0;color:white;background: rgba(0, 0, 0, 0.5);"}

"I cannot overstate how much I appreciated the focus on the real applications. The theory of research is important, but the focus on doing was truly empowering."

:::

:::

:::

:::

::: {.quarto-listing .quarto-listing-container-grid }
::: {.list .grid .quarto-listing-cols-2 .bg-info-light style="border-radius:10px;padding:10px;padding-top:20px;margin-top:20px;"}
::: {.g-col-1}

## Is PEER for you?

Our collaborators and hosts at the University of Cologne made a promotional video for their [PEER field school](http://www.google.com/url?q=http%3A%2F%2Fzus.uni-koeln.de%2Fpeer.html&sa=D&sntz=1&usg=AFQjCNHnBsqbSNnmTPRlNQj3fVI_7jqRFQ).

:::

::: {.g-col-1 style="padding-right:10px;"}

{{< video https://youtu.be/wYVePjWPDoc >}}

:::

:::

:::

:::{.bg-info style="border-radius:10px;padding:10px;"}

## What does PEER look like?

In a PEER field school, you'll work in a supportive community to build your skills and your research project along several overlapping development arcs:

:::{.quarto-listing .quarto-listing-container-grid .list .grid .quarto-listing-cols-3}

:::{.g-col-1 .card .card-body .text-center .bg-info .border-0}

:::{.bi .bi-ladder  style="font-size: 70px"}

:::
::: {style="font-size: 135%;"}
**Fundamental Skills**
:::
Generative writing, having a growth mindset, playing “yes, and”, mentoring students, researching as a parallel process, and iterating
:::

:::{.g-col-1 .card .card-body .text-center .bg-info .border-0}

:::{.bi .bi-columns-gap style="font-size: 70px"}

:::
::: {style="font-size: 135%;"}
**Research Design**
:::
Refining research questions, choosing theoretical frameworks, matching data streams to research questions and analysis methods, planning research projects, and building resilient and flexible projects.
:::

:::{.g-col-1 .card .card-body .text-center .bg-info .border-0}

:::{.bi .bi-easel style="font-size: 70px"}

:::
::: {style="font-size: 135%;"}
**Research Communication**
:::
Encapsulating your project, developing an elevator pitch, reading research literature, publishing peer-reviewed papers, negotiating authorship, finding collaborators, and giving robust constructive feedback on each other’s projects.
:::
:::
:::

&nbsp;  

::: {.quarto-listing .quarto-listing-container-grid .list .grid .quarto-listing-cols-2 style="height:500px;"}

:::{.g-col-1}

## Does PEER sound like a lot?

It is.  PEER field schools are intense, experiential, and supportive. You don't just learn these things abstractly; you actively practice them with your project. You are ready for PEER when you are enthusiastic about stretching your skills, refining your project, and getting energized about doing research. 

:::

::: {.g-col-1 .text-center style="margin-left:40px;margin-right:40px;background-image:url(../images/action-shots/facilitator_explaining.png);background-repeat:no-repeat;background-size: cover;position: relative;"}

:::{style="position:absolute;bottom:0;color:white;background: rgba(0, 0, 0, 0.5);"}

"I cannot overstate how much I appreciated the focus on the real applications. The theory of research is important, but the focus on doing was truly empowering."

:::

:::
:::

`r invitation.cards(c("announcements", "upcoming"))`
